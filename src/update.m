...
eIIvp = max(1e-6,eII-abs(tII-tIIo)./(De*dt + etamin));                     % get visco-plastic strain rate
...
yield  = max(1e-2,Ty + p);                                                 % get Griffith yield stress
...
for d = 1:ceil(delta)                                                      % regularise visco-plasticity
    dd   = delta/ceil(delta);
    etay = etay + dd.*(diff(etay([end-2,1:end,3],:),2,1)./8 ...
                     + diff(etay(:,[1,1:end,end]  ),2,2)./8);
end
etay   =  exp(etay);
...
dtU = ((eta(:,1:end-1) +  eta(:,2:end))./2./(h/2)^2 ...
    + (zeta(:,1:end-1) + zeta(:,2:end))./2./(h/2)^2).^-1;                  % iterative step size
dtP = (1./eta + K./(h/2)^2).^-1./1;                                        % iterative step size
