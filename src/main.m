...
Div_V  =  0.*P;  Div_v = 0.*P;  Div_fV = 0.*P;
...
if     restart < 0  % restart from last continuation frame
    ...
    load([name,'.mat'],'U','W','P','u','w','p','f','etay','eta','zeta','chi','K','exx','ezz','exz','txx','tzz','txz','eII','tII','Div_V','Div_v','time','step','fmass0');
...
while time <= tend && step <= M
    ...
    while resnorm/resnorm0 >= rtol && resnorm >= atol && it <= maxit || it <= nup
        ...
        w(:,[1 end]) = w(:,[end-1 2]);                                     % periodic boundaries
