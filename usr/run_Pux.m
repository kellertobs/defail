...
for bb = 1:length(BB)
    for pu = 1:length(PU)
        ...
        % set run parameters
        runID    = ['PuX',num2str(abs(PU(pu))),'_B',num2str(BB(bb)),'_N',num2str(NN)]; runID(runID=='.') = '_'; % run identifier
        ...
        Pu       =  PU(pu);              % ratio of pure-shear stress to buoyancy pressure
        ...
        gamma    =  0.60;                % iterative damping parameter (fraction of pre-previous step) [0,1]
        delta    =  (1-0.1/B^0.9);       % iterative relaxation for rheology updates [0,1]
